...
def run_cnn_with_results_for_target_column(index_col, train_images, 
                                           ...
                                           test_targets, test_images1, test_extra_predictors1, 
                                           test_targets1, test_images2, test_extra_predictors2, 
                                           test_targets2, test_images3, test_extra_predictors3, 
                                           test_targets3, test_images4, test_extra_predictors4, test_targets4):
    ...
    dataset_test1 = CustomImageDataset(raster_images=test_images1, extra_predictors = test_extra_predictors1, targets=test_targets1[:,index_col])
    dataloader_test1 = torch.utils.data.DataLoader(dataset_test1, batch_size=32)
    
    dataset_test2 = CustomImageDataset(raster_images=test_images2, extra_predictors = test_extra_predictors2, targets=test_targets2[:,index_col])
    dataloader_test2 = torch.utils.data.DataLoader(dataset_test2, batch_size=32)
    
    dataset_test3 = CustomImageDataset(raster_images=test_images3, extra_predictors = test_extra_predictors3, targets=test_targets3[:,index_col])
    dataloader_test3 = torch.utils.data.DataLoader(dataset_test3, batch_size=32)
    ...
    for i, data in enumerate(dataloader_test1):
        inputs, extra_predictors, labels = data
        inputs = inputs.clone().detach().to(device)
        labels = labels.clone().detach().to(device)
        extra_predictors = extra_predictors.clone().detach().to(device)
    ...
    preds = np.array(preds)
    all_labels = np.array(all_labels)
    rmse1 = compute_rmse(preds, all_labels)
    
    running_loss = []
    preds =  []
    all_labels = []
    
    for i, data in enumerate(dataloader_test2):
        inputs, extra_predictors, labels = data
        inputs = inputs.clone().detach().to(device)
        labels = labels.clone().detach().to(device)
        extra_predictors = extra_predictors.clone().detach().to(device)
    ...
    preds = np.array(preds)
    all_labels = np.array(all_labels)
    rmse2 = compute_rmse(preds, all_labels)
    
    running_loss = []
    preds =  []
    all_labels = []
    
    for i, data in enumerate(dataloader_test3):
        inputs, extra_predictors, labels = data
        inputs = inputs.clone().detach().to(device)
        labels = labels.clone().detach().to(device)
        extra_predictors = extra_predictors.clone().detach().to(device)
    ...
    preds = np.array(preds)
    all_labels = np.array(all_labels)
    rmse3 = compute_rmse(preds, all_labels)
    
    running_loss = []
    preds =  []
    all_labels = []
    
    for i, data in enumerate(dataloader_test4):
        inputs, extra_predictors, labels = data
        inputs = inputs.clone().detach().to(device)
        labels = labels.clone().detach().to(device)
        extra_predictors = extra_predictors.clone().detach().to(device)
    ...
    preds = np.array(preds)
    all_labels = np.array(all_labels)
    rmse4 = compute_rmse(preds, all_labels)
...
def run_cnn_with_results(data, seed):
    ...
    test_images1, test_extra_predictors1 = transform_data(test_images1, test_extra_predictors1, image_scaler, image_imputer, extra_predictor_scaler, extra_predictor_imputer)
    test_images2, test_extra_predictors2 = transform_data(test_images2, test_extra_predictors2, image_scaler, image_imputer, extra_predictor_scaler, extra_predictor_imputer)
    test_images3, test_extra_predictors3 = transform_data(test_images3, test_extra_predictors3, image_scaler, image_imputer, extra_predictor_scaler, extra_predictor_imputer)
    test_images4, test_extra_predictors4 = transform_data(test_images4, test_extra_predictors4, image_scaler, image_imputer, extra_predictor_scaler, extra_predictor_imputer)
    results = []
    ...
    result = run_cnn_with_results_for_target_column(2, train_images, 
                                           ...
                                           test_targets3, test_images4, test_extra_predictors4, test_targets4)
    results.append(result)
    result = run_cnn_with_results_for_target_column(3, train_images, 
                                           train_extra_predictors, train_targets, 
                                           val_images, val_extra_predictors, 
                                           val_targets, test_images, test_extra_predictors, 
                                           test_targets, test_images1, test_extra_predictors1, 
                                           ...
                                           test_targets3, test_images4, test_extra_predictors4, test_targets4)
    results.append(result)
    result = run_cnn_with_results_for_target_column(4, train_images, 
                                           train_extra_predictors, train_targets, 
                                           val_images, val_extra_predictors, 
                                           val_targets, test_images, test_extra_predictors, 
                                           test_targets, test_images1, test_extra_predictors1, 
